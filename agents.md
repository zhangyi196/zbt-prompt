...
- `组图 23.md`：Ref-A/B 到 Target 差异迁移；固定六板块，List 1 / List 2 各 16 条；不得削弱 Ref 绑定、冷区避让、人物互斥、空间可放置性和禁改规则。
...
流程：`组图 23 表情前置.md` 输出类别 -> App 按类别补全眉/眼/嘴 -> `组图 23.md` 按剧情适配完整差异。前置默认三组：负向组、正向组、正负向组；单人时正负向组不适用，多人时每组 2-3 人且正负向组至少 1 正 1 负。`组图 23.md` 只能以 Ref-A -> Ref-B 的真实可见变化为准；前置预案和用户表情描述只作参考。剧情字段要输出简短自然小剧情，不输出 `->` 箭头链路或用户原文复述。List 2 边角覆盖低于可执行性；四角或贴边区域没有完整可放置空间时，改用已有独立小物体的局部变化，不硬塞新增云朵、装饰球或飞鸟。
