...
表情历史约定：`draw_history.json.expression_pools` 用独立计数做历史降权；类别 key 形如 `expression_category:{极性}:{单人/多人}`，模板 key 形如 `expression_template:{极性}:{单人/多人}:{具体表情}`。多候选 `具体表情` 和随机模板都按“使用次数越少权重越高”选择；单一类别或指定模板编号在增强成功后同样记历史。`重置全部历史` 会同时清空 `item_pools`、`animal_pools`、`expression_pools`，单独重置物品/动物按钮不影响表情历史。
