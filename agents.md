...
- `Game content extraction/data/agents.md`、`Game content extraction/data/审查规则.md`、`Game content extraction/data/审查文件.md`、`Game content extraction/data/审查文件/{类别} 审查文档.md`、`Game content extraction/data/修改规则.md`：盲盒四池写库、类别审查记录和审查后替换细则，改 `blind_boxes.py` 前先读。
...
- 修改盲盒物品库时先用 `Game content extraction/data/审查规则.md` 发现问题并写入 `Game content extraction/data/审查文件/{类别} 审查文档.md`，再按 `Game content extraction/data/修改规则.md` 读取对应类别审查文档后替换；替换必须解决视觉本体重复、场景常识错位和同模板凑数的根因。
...
- 第四池优先写辅助中型实体，如抽取器、托盘、夹、筒、篮、垫、桶、小设备、清洁维护物；条目还必须通过 `test_scene_expansion_items_keep_medium_scene_fragment_boundary` 的中型边界，裸物若边界不稳，应改成收纳筒、托盘、支架、篮、座、器、刷、夹等稳定单物。
- 不得写大型家具、过小配件、裸露标签卡片，也不得用 `功能词 + 板/盘/垫/盒/包/册/卷` 临时造词；项目禁词如 `流苏` 即使在手作场景也不得进入物品池。
- 写库时先按真实物品族选物，再检查尾词配额和雷同密度；禁止 `收纳盒`、`整理篮`、`展示架`、`托座`、`分装盒`、`xxx清洁刷`、`xxx密封袋`、`xxx晾干架`、`xxx袋` 等模板凑数。
- 默认面向欧美 / 国际化找不同用户；除非类别明确要求，不写强中式、强地域或强课程体系绑定物品。商店零食类优先薯片、椒盐脆饼、饼干、巧克力、糖果、爆米花、坚果、谷物棒、果干、牛奶、果汁、汽水等通用物，不写 `山楂片`、`仙贝`、`豆干`、`鱿鱼丝`、`鱼皮零食` 等强地域零食。
