...
- `prompts/group-image/组图 23 表情前置.md`、`组图 23 表情库.md`、`组图 23 表情库 版本管理.md`：人物表情前置规则、事实源与版本记录；其中表情库固定留在根目录。
- `美式卡通具体表情写法.md`：表情库趣味化重构草案；顶层保留简洁情绪词，每类下挂 8 个具体卡通脸部变体，模板采用主视觉锚点优先和灵活脸部字段写法。
- `表情库 2.0.0版本.md`：人物表情库 2.0.0 目录来源草案；正式事实源已同步到根目录 `组图 23 表情库.md`，后续以正式库为准。
