...
盲盒物品内容库优化方向已记录在 `../.workflow/.brainstorm/BS-2026-04-29-优化game-content-extraction盲盒物品内容/`，完整规格包见 `../.workflow/.spec/SPEC-2026-04-29-game-content-extraction-blind-box-library-refactor/`。后续重写 `data/blind_boxes.py` 时，优先采用 20 个 `常见场景+用途` 入口：`桌面+学习`、`餐桌+茶歇`、`厨房+烘焙`、`卧室+梳妆`、`浴室+洗护`、`客厅+装饰`、`儿童房+玩具`、`宠物+日常`、`庭院+园艺`、`门口+雨具`、`沙滩+度假`、`公园+野餐`、`营地+露营`、`街道+出行`、`运动场+装备`、`海底+潜水`、`节日+礼物`、`手作+缝纫`、`手作+编织`、`商店+零食`。每类内容按 `core_items`、`support_items`、`visible_small_items`、`conditional_items`、`blocked_or_risky` 五层组织；若暂不改 UI，可映射回现有 `large` / `medium` / `small` / `hanging`。

已落地试点盒号：15 `桌面+学习`、16 `海底+潜水`、17 `公园+野餐`。`data/blind_boxes.py` 中 `BLIND_BOX_SCENE_ENTRIES` 是 20 类入口清单，`BLIND_BOX_ITEM_POOL_BUNDLES` 是三类五层池，`BLIND_BOX_COMPATIBILITY_MAPPING` 说明五层到四栏来源；`BLIND_BOXES` 仍是运行时消费的四栏兼容视图。

条件池 / 风险池物品化修正已按 `../.workflow/.spec/SPEC-2026-04-29-blind-box-pool-itemization-fix/` 落地。后续维护三类试点时，五层池都必须是具体物品或物品组；`conditional_items` 必须是中等以上体量、可单独圈选的条件启用物品；`blocked_or_risky` 只保留具体风险物；折线、擦痕、气泡、阴影、边线、微小颗粒等非物品内容只能写入 forbidden patterns 测试 / 审查规则，不得作为池条目。

物品写库验收：核心物必须一眼可识别、边界清楚、可单独圈选、不依赖光影/反射/透明效果、不靠单点细线微型痕迹，并与场景强相关；小物必须成组、块状或有明确承载；悬挂、墙面、软布、灯、镜、透明反光、细绳、流苏、微型痕迹和动物本体默认放入条件池或风险池，不进入默认主抽取池。
...
- 目标模型固定为 20 个 `场景+用途` 入口，内容层级为 `core_items`、`support_items`、`visible_small_items`、`conditional_items`、`blocked_or_risky`。
- 运行时短期仍以 `large` / `medium` / `small` / `hanging` 四栏工作；实现时先做五层到四栏的兼容映射，不直接跳过这层约束。
...
- `test_blind_box_content_model.py` 覆盖入口数量、五层 schema、风险池零泄漏、四栏兼容、旧输入覆盖、状态词过滤和非物品 forbidden patterns。
- 物品化补丁规格已完成并落地，入口文件优先读 `../.workflow/.spec/SPEC-2026-04-29-blind-box-pool-itemization-fix/spec-summary.md`；执行会话见 `../.workflow/active/WFS-blind-box-pool-itemization-fix/`。
- 重构规格 Readiness 评分为 92.75 / 100，物品化补丁 Readiness 评分为 95.25 / 100，Gate 均为 Pass；`issue-export-report.md` 已给出建议 issue。
