...
- 右上角 `检查更新` 按钮读取 GitHub Releases；当前版本 `APP_VERSION = "0.1.1"`，仓库为 `zhangyi196/zbt-prompt`。
...
`APP_VERSION`、`UPDATE_API_URL`、`UPDATE_RELEASES_URL` 定义在 `内容抽取.py` 顶部。当前值：
...
更新按钮固定在主窗口右上角；点击后后台线程访问 GitHub latest release API，比较版本并弹窗提示。有新版时只打开 Releases 页面，不自动下载、不覆盖当前 exe、不重启程序。

当前 GitHub Releases 可能为空；API 返回 404 时应中文提示“当前 GitHub Releases 还没有发布文件”，并允许打开发布页。更新检查不得阻塞 UI，不得写入 `draw_history.json`，不得影响盲盒/动物和人物表情抽取。除非新增本地版本文件，否则 `内容抽取.spec` 不需要为更新功能新增 datas。

发布新版时，先更新 `APP_VERSION`，再创建高于当前版本的 GitHub Release tag，例如 `v0.1.2` 或 `v1.0.0`；tag 可带 `v`，代码会抽取数字版本比较。若未来要支持自动下载或自动替换，必须新增独立 updater/helper、校验和失败回滚策略。
...
- 不把检查更新改成自动替换 exe；若未来要自动更新，必须新增独立 updater/helper、文件校验和失败回滚策略。
