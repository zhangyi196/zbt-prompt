...
- 右上角 `检查更新` 按钮读取 GitHub Releases；版本和仓库地址见“检查更新与发布”。
...
`内容抽取.py` 顶部维护版本和发布地址：`APP_VERSION = "0.1.1"`，`UPDATE_API_URL = "https://api.github.com/repos/zhangyi196/zbt-prompt/releases/latest"`，`UPDATE_RELEASES_URL = "https://github.com/zhangyi196/zbt-prompt/releases"`。

右上角 `检查更新` 后台访问 latest release API，比较 tag 数字版本，只弹窗提示并打开 Releases 页面；不得自动下载、覆盖 exe、重启程序、写入 `draw_history.json` 或阻塞 UI。API 404 时提示“当前 GitHub Releases 还没有发布文件”并允许打开发布页。
