...
- `../组图 23.md`：差异迁移主提示词；当前为压缩结构版，维护时避免恢复重复表述；维护剧情字段时保持简短自然小剧情，不输出 `->` 箭头链路或用户原文复述；维护 List 1 时必须保留同一人物面部、手中物、脸侧符号、后背标识和身后紧邻对象按同一热区互斥的约束；维护 List 2 时必须保留冷区避让、每块冷区用已有独立小物或局部承载面兜底、边角覆盖低于可执行性、新增物体空间可放置性、以及灯具照明关系不可被整体消失破坏的约束。
