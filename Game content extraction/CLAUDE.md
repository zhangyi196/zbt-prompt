...
- `../组图 23.md`：差异迁移主提示词；当前 v1.7.10.6 为压缩结构版，维护时避免恢复重复表述；维护剧情字段时保持简短自然小剧情，不输出 `->` 箭头链路或用户原文复述；维护 List 1 时保留人物贴邻热区互斥；维护 List 2 时保留冷区避让、局部承载面兜底、边角覆盖低于可执行性、新增物体空间可放置性和灯具照明关系约束。
