...
条件池 / 风险池物品化修正规格见 `../.workflow/.spec/SPEC-2026-04-29-blind-box-pool-itemization-fix/`。后续维护三类试点时，五层池都必须是具体物品或物品组；`conditional_items` 必须是中等以上体量、可单独圈选的条件启用物品；`blocked_or_risky` 只保留具体风险物；折线、擦痕、气泡、阴影、边线、微小颗粒等非物品内容只能写入 forbidden patterns 测试 / 审查规则，不得作为池条目。
...
- `test_blind_box_content_model.py` 覆盖入口数量、五层 schema、风险池零泄漏、四栏兼容、旧输入覆盖和状态词过滤。
- 物品化补丁规格已完成，入口文件优先读 `../.workflow/.spec/SPEC-2026-04-29-blind-box-pool-itemization-fix/spec-summary.md`；后续实现先替换 15 / 16 / 17 三类试点的 `conditional_items` 和 `blocked_or_risky`，再加 forbidden patterns 测试。
- Readiness 评分为 92.75 / 100，Gate 为 Pass；`issue-export-report.md` 已给出 4 个建议 issue。
