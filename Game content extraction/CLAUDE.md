...
盲盒物品内容库优化方向已记录在 `../.workflow/.brainstorm/BS-2026-04-29-优化game-content-extraction盲盒物品内容/`，完整规格包见 `../.workflow/.spec/SPEC-2026-04-29-game-content-extraction-blind-box-library-refactor/`。后续重写 `data/blind_boxes.py` 时，优先采用 20 个 `常见场景+用途` 入口：`桌面+学习`、`餐桌+茶歇`、`厨房+烘焙`、`卧室+梳妆`、`浴室+洗护`、`客厅+装饰`、`儿童房+玩具`、`宠物+日常`、`庭院+园艺`、`门口+雨具`、`沙滩+度假`、`公园+野餐`、`营地+露营`、`街道+出行`、`运动场+装备`、`海底+潜水`、`节日+礼物`、`手作+缝纫`、`手作+编织`、`商店+零食`。最新四池规格见 `../.workflow/.spec/SPEC-2026-04-29-game-content-extraction-four-pool-refactor/`，目标内容池为 `core_items`、`support_items`、`visible_small_items`、`scene_expansion_items`。

已落地试点盒号：15 `桌面+学习`、16 `海底+潜水`、17 `公园+野餐`。`data/blind_boxes.py` 中 `BLIND_BOX_SCENE_ENTRIES` 是 20 类入口清单；`BLIND_BOX_ITEM_POOL_BUNDLES` 已是三类四池模型；`BLIND_BOXES` 仍是运行时消费的四栏兼容视图。

四类内容池重构已按 `../.workflow/.spec/SPEC-2026-04-29-game-content-extraction-four-pool-refactor/` 落地，执行会话见 `../.workflow/active/WFS-game-content-four-pool-refactor/`。后续维护三类试点时，`conditional_items`、`anchor_required_items`、`blocked_or_risky` 不再作为目标候选池；第四类统一为 `scene_expansion_items`，即中等以上、无图像锚点依赖、能自然增加场景变化的场景扩展物。
...
- 目标模型固定为 20 个 `场景+用途` 入口；三类试点下一步内容层级收敛为 `core_items`、`support_items`、`visible_small_items`、`scene_expansion_items`。
- 运行时短期仍以 `large` / `medium` / `small` / `hanging` 四栏工作；实现时先做四池到四栏的兼容映射，不直接跳过这层约束。
- 首批只验证 `桌面+学习`、`海底+潜水`、`公园+野餐` 三类；当前对应盒号为 15 / 16 / 17，不要一开始就全量改完 20 类。
- `item_states.py` 中带透明、反光、高光语义的状态词视为风险；`内容抽取.py` 目前会过滤 `半透明`、`高光反光`、`带有光泽` 这些物品状态关键词。
- `test_blind_box_content_model.py` 当前覆盖入口数量、四池 schema、旧条件 / 风险池移除、四栏兼容、旧输入覆盖、状态词过滤和 `blocked_patterns`。
