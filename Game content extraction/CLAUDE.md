...
- 右上角 `检查更新` 按钮读取 GitHub Releases；当前版本 `APP_VERSION = "0.1.0"`，仓库为 `zhangyi196/--1----`。
...
发布新版时，先更新 `APP_VERSION`，再创建高于当前版本的 GitHub Release tag，例如 `v0.1.1` 或 `v1.0.0`；tag 可带 `v`，代码会抽取数字版本比较。若未来要支持自动下载或自动替换，必须新增独立 updater/helper、校验和失败回滚策略。
