...
- 表情区按钮顺序为 `表情统计`、`清空输入`、`抽取表情`、`复制结果`、`降低统计`；`表情统计` 会提交当前输入最后一次成功抽取的表情结果，并把完整 48 类统计摘要复制到剪贴板，不覆盖增强后文本；`降低统计` 是人工校准入口，弹窗内单选 `正向` 或 `负向` 并输入正整数次数，一次只降低所选极性的全部正式类别，不自动提交当前暂存结果。
...
- 表情统计写入 `expression_stats.json`：同一组输入可多次抽取，但只以最后一次成功抽取为暂存；点击 `表情统计`、切换到新输入抽取或关闭 App 时提交暂存结果；同一组已提交结果再次抽取后会用新的最后结果替换本组旧统计。`降低统计` 只修改 `committed_counts`，不修改 `draw_history.json.expression_pools`。
