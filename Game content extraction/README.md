...
- `人物表情抽取`：把 `具体表情` 类别补全为具体表情变体和脸部字段描述。
...
当前用于检查更新测试的版本对：`v0.1.7 -> v0.1.8`。两者功能保持一致，主要用于验证安装包内更新流程。

- Release 页面：<https://github.com/zhangyi196/zbt-prompt/releases>
- 测试安装包：`GameContentExtraction-Setup-v0.1.7.exe`、`GameContentExtraction-Setup-v0.1.8.exe`
...
历史保存在 `draw_history.json`：物品使用 `item_pools`，动物使用 `animal_pools`，人物表情使用独立 `expression_pools`。重置按钮分别清空物品、动物或全部抽取历史；表情历史只参与表情降权，不混入物品 / 动物池。表情前置反馈统计另存为 `expression_stats.json`，不并入 `draw_history.json`。
...
表情工作区读取 `极性`、`具体表情`、`单人/多人`，从 `../组图 23 表情库.md` 查模板，并只增强 `具体表情:` 字段。模板行兼容旧格式 `1. 眉：……`，也支持新格式 `1. 气到脸红；字段：……；字段：……`；新格式可从 `眉`、`眼`、`脸颊`、`额头`、`嘴`、`牙齿`、`舌头`、`口边`、`脸侧` 任一脸部字段开头，并会把具体表情变体和脸部字段一起写回结果。前置提示词可在同一 `具体表情` 一栏给出多个候选类别；当前前置默认输出 2 个精准候选类别。App 支持用 `、`、`，`、`,`、`/`、`|` 分隔候选，先校验候选都存在且极性一致，再按历史降权随机择一类别补全。正式库包含正向 24 类、负向 24 类，每类模板分为单人 1-8、多人 1-8，强度从中等可见起步，不保留轻微档；随机模板也按历史降权选择，仍可指定编号。

核心规则：

- `组图 23 表情库.md` 是单一事实源。
- 支持同一输入中的多组 `极性:`。
- 前置提示词的 `具体表情:` 一栏可包含多个候选类别；当前前置默认输出 2 个精准候选类别。App 会先整组校验，再按 `weight = 1 / (used_count + 1)` 从候选中加权选中 1 个类别写回最终结果。
- 重复增强会替换旧表情模板，不会堆叠眉 / 眼 / 脸颊 / 额头 / 嘴等脸部字段。
- “清空输入”只清空表情输入框，不清空输出框；若盲盒区“自动粘贴”处于开启状态，会像盲盒区一样把剪贴板内容回填到表情输入框，方便连续测试。
- 表情区按钮顺序为 `表情统计`、`清空输入`、`抽取表情`、`复制结果`；`表情统计` 会提交当前输入最后一次成功抽取的表情结果，并把完整 48 类统计摘要复制到剪贴板，不覆盖增强后文本。
- 页面只显示 `具体表情查看` 框，不显示增强后文本框；`具体表情查看` 只解析内部增强后文本中的 `人物定位` 和增强后的 `具体表情`，方便快速检查；`复制结果` 仍只复制增强后文本，不包含查看框。
...
- 表情统计写入 `expression_stats.json`：同一组输入可多次抽取，但只以最后一次成功抽取为暂存；点击 `表情统计`、切换到新输入抽取或关闭 App 时提交暂存结果；同一组已提交结果再次抽取后会用新的最后结果替换本组旧统计。

新格式模板编号 `1` 的示例输出：

```text
具体表情: 委屈，小鸭嘴瘪住；嘴：嘴巴向前瘪成小鸭嘴形；眼：双眼湿漉漉看向[目标物]；脸颊：双颊被瘪嘴挤得微微鼓起；眉：双眉向内上拱。
```
