...
表情工作区读取 `极性`、`具体表情`、`单人/多人`，从 `../组图 23 表情库.md` 查模板，并只把眉 / 眼 / 嘴追加到 `具体表情:` 后。前置提示词会在同一 `具体表情` 一栏给出 4 个候选类别；交给 App 增强时需先从中择一类别或拆分为单一类别输入。每类 8 条模板：1-4 单人，5-8 多人；可随机或指定编号。

核心规则：

- `组图 23 表情库.md` 是单一事实源。
- 支持同一输入中的多组 `极性:`。
- 前置提示词的 `具体表情:` 一栏可能包含 4 个候选类别；当前 App 增强仍按单一类别匹配。
- 重复增强会替换旧眉 / 眼 / 嘴，不会堆叠。
