...
表情工作区读取 `极性`、`具体表情`、`单人/多人`，从 `../组图 23 表情库.md` 查模板，并只把眉 / 眼 / 嘴追加到 `具体表情:` 后。前置提示词可在同一 `具体表情` 一栏给出 4 个候选类别；App 支持用 `、`、`，`、`,`、`/`、`|` 分隔候选，先校验候选都存在且极性一致，再按历史降权随机择一类别补全。每类 8 条模板：1-4 单人，5-8 多人；随机模板也按历史降权选择，仍可指定编号。
