...
维护规则见本目录 [agents.md](agents.md)；盲盒物品写库规则见 [data/agents.md](data/agents.md)；全仓提示词规则见上级目录 [agents.md](../agents.md)。
...
每个类别下按四类内容池维护：`core_items` 写主题核心主体物，`support_items` 写配套工具物，`visible_small_items` 写成组可见小物，`scene_expansion_items` 保留历史字段名但按可放置中型单物维护。运行时继续把四池内容映射回现有大型 / 中型 / 小型 / 悬挂四栏，其中悬挂栏保留为空兼容桶。详细写库规则见 `data/agents.md`。

当前 20 个盲盒盒号均已按四类内容池重构，旧静态盲盒物品内容已从数据文件移除；抽取物品直接来自四池内容，并继续映射回现有大型 / 中型 / 小型 / 悬挂四栏。写库时不得靠 `收纳盒`、`整理篮`、`展示架`、`托座`、`分装盒`、`组合` 等尾词矩阵凑数；`scene_expansion_items` 不再写组合，优先写适合找不同新增、删除、替换的中型单物，并按真实物品族分布和尾词配额控制，避免 `功能词 + 板/盘/垫/盒/包/册/卷` 成片造词。盒号 1-20 保留用于逗号输入和历史兼容，但语义已切换为新的场景+用途目录。启用物品状态时，程序会过滤 `半透明`、`高光反光`、`带有光泽` 等风险状态词。
