...
表情工作区读取 `极性`、`具体表情`、`单人/多人`，从 `../组图 23 表情库.md` 查模板，并只增强 `具体表情:` 字段。模板行兼容旧格式 `1. 眉：……`，也支持新格式 `1. 气到脸红；眉：……；眼：……；脸颊：……；嘴：……`；新格式会把具体表情变体和脸部字段一起写回结果。前置提示词可在同一 `具体表情` 一栏给出 8 个候选类别；App 支持用 `、`、`，`、`,`、`/`、`|` 分隔候选，先校验候选都存在且极性一致，再按历史降权随机择一类别补全。每类模板分为单人 1-8、多人 1-8，强度从中等可见起步，不保留轻微档；随机模板也按历史降权选择，仍可指定编号。

核心规则：

- `组图 23 表情库.md` 是单一事实源。
- 支持同一输入中的多组 `极性:`。
- 前置提示词的 `具体表情:` 一栏可包含 8 个候选类别；App 会先整组校验，再按 `weight = 1 / (used_count + 1)` 从候选中加权选中 1 个类别写回最终结果。
