...
表情工作区读取 `极性`、`具体表情`、`单人/多人`，从 `../组图 23 表情库.md` 查模板，并只增强 `具体表情:` 字段。模板行兼容旧格式 `1. 眉：……`，也支持新格式 `1. 气到脸红；眉：……；眼：……；脸颊：……；嘴：……`；新格式会把具体表情变体和脸部字段一起写回结果。前置提示词可在同一 `具体表情` 一栏给出 4 个精准候选类别；App 支持用 `、`、`，`、`,`、`/`、`|` 分隔候选，先校验候选都存在且极性一致，再按历史降权随机择一类别补全。每类模板分为单人 1-8、多人 1-8，强度从中等可见起步，不保留轻微档；随机模板也按历史降权选择，仍可指定编号。
...
```text
具体表情: 生气，气到脸红；眉：一侧眉头压低，另一侧眉尾挑起；眼：一侧眼瞪住[目标物]，另一侧眼收窄；脸颊：两侧脸颊明显涨红；嘴：嘴巴抿紧成下压斜线。
```
