...
class UpdateCheckTests(unittest.TestCase):
    ...
    def make_release_payload(self, version="0.2.1", assets=None, **overrides):
        payload = {
    ...
    def assert_manual_download_required(self, result, *, version="0.2.1", url):
    ...
    def test_latest_404_falls_back_to_releases_list(self):
                    ...
                    [
                        ...
                        self.make_release_payload(
                            ...
                            assets=[
                                ...
                                self.make_asset(
                                    "GameContentExtraction-Setup-v0.2.1.exe",
                                    url="https://example.test/downloads/installer-v0.2.1.exe",
                                    size=456789,
                                ),
                            ],
                        ),
        ...
        self.assertEqual(result["url"], "https://example.test/releases/v0.2.1")
        self.assert_installer_metadata(
            result,
            name="GameContentExtraction-Setup-v0.2.1.exe",
            url="https://example.test/downloads/installer-v0.2.1.exe",
            size=456789,
        )
    ...
    def test_latest_403_and_429_fall_back_to_latest_release_redirect(self):
        redirect_url = "https://github.com/zhangyi196/zbt-prompt/releases/tag/v0.2.1"
        ...
        result = extractor._build_update_result(
            self.make_release_payload(
                ...
                assets=[
                    ...
                    self.make_asset(
                        "GameContentExtraction-Setup-v0.2.1.exe",
                        url="https://example.test/downloads/setup-v0.2.1.exe",
                        size=987654,
                    ),
                ],
            )
    ...
    def test_extract_installer_asset_prefers_setup_executable(self):
        ...
        self.assertEqual(selected["name"], "GameContentExtraction-Setup-v0.2.1.exe")
        self.assertEqual(
            selected.get("download_url") or selected.get("browser_download_url") or selected.get("url"),
            "https://example.test/downloads/setup-v0.2.1.exe",
        )
    ...
    def test_extract_installer_asset_without_assets_reports_manual_download(self):
        ...
        selected, error = extractor._extract_installer_asset(
            self.make_release_payload("0.2.1")
        )
    ...
    def test_select_installer_asset_metadata_prefers_setup_exe_and_skips_invalid_candidates(self):
        ...
        selected = extractor._select_installer_asset_metadata(
            [
                self.make_asset("notes.txt"),
                self.make_asset(
                    "GameContentExtraction-v0.2.1.exe",
                    url="https://example.test/downloads/generic-v0.2.1.exe",
                ),
                self.make_asset(
                    "GameContentExtraction-Setup-v0.2.1.exe",
                    url="https://example.test/downloads/setup-v0.2.1.exe",
                ...
                self.make_asset(
                    "GameContentExtraction-portable-v0.2.1.exe",
                    url="https://example.test/downloads/portable-v0.2.1.exe",
                ),
                self.make_asset(
                    "GameContentExtraction-v0.2.1.msi",
                    url="https://example.test/downloads/setup-v0.2.1.msi",
                ),
                self.make_asset(
                    "GameContentExtraction-Setup-v0.2.1.exe",
