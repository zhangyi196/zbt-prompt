...
- `主图 第一步.md`：与仓库根目录同名规则文件保持同步；先判断画面是否需要新增内容，若原图已完整或无安全空间则固定输出 `不需要新增物品`，只有需要新增时才输出 2 个盲盒类别编号、动物字段、禁用字段和补偿字段。该提示词已同步 20 类新盲盒目录、类别物品口径、类别内容速查和场景硬约束，不再使用旧 14 类主题库。
...
- `../组图 23.md`：差异迁移主提示词；当前 v1.7.10.8 为中等压缩结构版，维护时避免恢复重复表述；维护剧情字段时保持简短自然小剧情，不输出 `->` 箭头链路或用户原文复述；维护 List 1 时保留人物贴邻热区互斥；维护 List 2 时保留冷区避让、局部承载面兜底、边角覆盖低于可执行性、新增物体空间可放置性和灯具照明关系约束。
...
盲盒物品内容库优化方向已记录在 `../.workflow/.brainstorm/BS-2026-04-29-优化game-content-extraction盲盒物品内容/`，完整规格包见 `../.workflow/.spec/SPEC-2026-04-29-game-content-extraction-blind-box-library-refactor/`。`data/blind_boxes.py` 已采用 20 个 `常见场景+用途` 入口：`桌面+学习`、`餐桌+茶歇`、`厨房+烘焙`、`卧室+梳妆`、`浴室+洗护`、`客厅+装饰`、`儿童房+玩具`、`宠物+日常`、`庭院+园艺`、`门口+雨具`、`沙滩+度假`、`公园+野餐`、`营地+露营`、`街道+出行`、`运动场+装备`、`海底+潜水`、`节日+礼物`、`手作+缝纫`、`手作+编织`、`商店+零食`。最新四池规格见 `../.workflow/.spec/SPEC-2026-04-29-game-content-extraction-four-pool-refactor/`，目标内容池为 `core_items`、`support_items`、`visible_small_items`、`scene_expansion_items`。

已完成 20 类全量替换。`data/blind_boxes.py` 中 `BLIND_BOX_SCENE_ENTRIES` 是 20 类入口清单；`BLIND_BOX_ITEM_POOL_BUNDLES` 是全量四池模型；旧静态盲盒物品内容已移除，`BLIND_BOXES` 直接由新四池内容生成并作为运行时消费的四栏兼容视图。盒号 1-20 保留用于逗号输入和历史兼容，但语义已切换为新的场景+用途目录。

仓库根目录 `主图 第一步.md` 已同步同一套 20 类盲盒编号、类别物品口径、类别内容速查和场景硬约束；维护 App 内容库时，如果改变盒号语义，也要同步该提示词的类别库与示例。

四类内容池重构已按 `../.workflow/.spec/SPEC-2026-04-29-game-content-extraction-four-pool-refactor/` 落地，全量替换执行会话见 `../.workflow/.team/TLV4-2026-04-29-blind-box-content-replace/`。后续维护时，`conditional_items`、`anchor_required_items`、`blocked_or_risky` 不再作为目标候选池；第四类统一为 `scene_expansion_items`，即中等以上、无图像锚点依赖、能自然增加场景变化的场景扩展物。

物品写库验收：核心物必须一眼可识别、边界清楚、可单独圈选、不依赖光影/反射/透明效果、不靠单点细线微型痕迹，并与场景强相关；小物必须成组、块状或有明确承载；场景扩展物必须中等以上、无需画面已有对象成立；细绳、流苏、边线、折线、擦痕、阴影、高光、反光、透明、发光、动物本体、微小痕迹只进入 `blocked_patterns` 测试 / 审查规则。
...
- 规格文档已完成于 `../.workflow/.spec/SPEC-2026-04-29-game-content-extraction-blind-box-library-refactor/`，入口文件优先读 `spec-summary.md`、`requirements/_index.md`、`architecture/_index.md`、`epics/_index.md`。
- 目标模型固定为 20 个 `场景+用途` 入口；当前 20 类均已收敛为 `core_items`、`support_items`、`visible_small_items`、`scene_expansion_items`。
- 运行时仍以 `large` / `medium` / `small` / `hanging` 四栏工作；四池到四栏的兼容映射必须保留，不直接跳过这层约束；`hanging` 当前为空兼容桶。
- 当前已全量替换 20 类并移除旧静态物品池；不要再按“三类试点”路线新增旧池内容。
- `item_states.py` 中带透明、反光、高光语义的状态词视为风险；`内容抽取.py` 目前会过滤 `半透明`、`高光反光`、`带有光泽` 这些物品状态关键词。
- `test_blind_box_content_model.py` 当前覆盖 20 类入口数量、全量四池 schema、旧条件 / 风险池移除、四栏兼容、旧输入覆盖、状态词过滤和 `blocked_patterns`。
- 四类内容池规格与执行已完成，入口文件优先读 `../.workflow/.spec/SPEC-2026-04-29-game-content-extraction-four-pool-refactor/spec-summary.md` 和 `../.workflow/active/WFS-game-content-four-pool-refactor/IMPL_PLAN.md`。
- 重构规格 Readiness 评分为 92.75 / 100，物品化补丁 Readiness 评分为 95.25 / 100，四池规格 Readiness 评分为 96.5 / 100，Gate 均为 Pass。

- 保持中文界面、中文输出和复制即用风格。
- 不改成 Web、数据库、服务端或大型工程。
- 不破坏输入语法、历史语义、重置按钮语义、表情库单一事实源和占位符保留。
- 若同步维护 `../组图 23.md`，剧情字段要改写为短小自然句；List 1 同一人物贴邻热区内不得叠加表情、脸侧符号、手中物、后背标识或身后紧邻屏幕；List 2 冷区无新增空间时要先用已有独立小物或局部承载面补点；新增 / 异物植入 / 替换为更大物体必须有完整可见且未被占用的空间，不能为了容纳差异点缩小、移动、遮挡或重排原图主体；灯具、灯罩、灯泡、蜡烛火焰等不得整体消失导致光照关系变化。
