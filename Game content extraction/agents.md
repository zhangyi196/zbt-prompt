...
- `data/agents.md`、`data/审查规则.md`、`data/审查文件.md`、`data/审查文件/{类别} 审查文档.md`、`data/修改规则.md`：盲盒四池写库、类别审查记录和审查后替换规则；改盲盒内容前先读。
...
写库验收：每池 50 条且池内唯一；条目必须是真实可见、单点差异、边界清楚、可单独圈选的物品或成单位物品集合。修改已写库内容时，先按 `data/审查规则.md` 找问题并写入 `data/审查文件/{类别} 审查文档.md`，再按 `data/修改规则.md` 读取对应类别审查文档后替换，必须解决视觉本体重复、场景常识错位和同模板凑数的根因。禁止靠 `收纳盒`、`整理篮`、`展示架`、`托座`、`分装盒`、`组合` 等尾词批量扩写，也要拦住 `xxx清洁刷`、`xxx袋`、`xxx架` 等同物品族换前缀凑数。同一基础对象不得在同一类别四池机械重复；若数量不足，优先扩展不同物品族，不用同质尾词凑数。生成时先按真实物品族选物，再检查反模板规则、尾词配额和雷同密度，避免 `功能词 + 板/盘/垫/盒/包/册/卷` 成片造词。禁止依赖光影、透明感、反光、高光、发光、细线或微型痕迹作为识别点；普通杯、瓶、罐可以写，但不得强调透明或反光特征。项目禁词按测试为准，例如 `流苏` 即使在手作场景也不得进入物品池。`conditional_items`、`anchor_required_items`、`blocked_or_risky` 不再作为候选池，风险内容只进 `blocked_patterns`。
...
- `scene_expansion_items`：保留历史字段名，实际语义为可放置中型单物；写适合找不同新增、删除、替换的中型物品，并且必须通过 `test_scene_expansion_items_keep_medium_scene_fragment_boundary` 的中型边界；裸物若边界不稳，应改成收纳筒、托盘、支架、篮、座、器、刷、夹等稳定单物。不写 `A+B组合`、`用品组合`、`小物组合`、`包装组合`，不写推车、抽屉柜、陈列架、货架、书桌、柜台等大型家具 / 存储主体；先覆盖多个真实物品族，再控制 `板/盘/垫/盒/包/册/卷` 等尾词配额。

商店零食类默认面向欧美 / 国际化用户：优先薯片、椒盐脆饼、饼干、巧克力、糖果、爆米花、坚果、谷物棒、果干、牛奶、果汁、汽水、冰茶、运动饮料等通用物；不写 `山楂片`、`仙贝`、`豆干`、`鱿鱼丝`、`鱼皮零食` 等强地域零食。
