...
- 改盲盒内容时，先看 `data/agents.md` 给出的阅读顺序，再看 `data/审查规则.md`、`data/审查文件.md`、对应类别审查文档和 `data/修改规则.md`；完成一轮替换后再读 `data/复查清单.md` 收尾复查。目标不是换词，而是解决场景错位、视觉本体重复、模板化和伪多样性；同族平移替换如 `作品记录本 -> 项目说明本` 不算修复；若只是给旧词前面缝场景词、口味词或材质词，如 `磅蛋糕 -> 柠檬磅蛋糕`、`茶包 -> 篝火茶包`，主词未变也视为未修复；若旧问题只被改写成另一串 `xx板 / xx架 / xx器 / xx机` 弱模板词，也视为未修复。
