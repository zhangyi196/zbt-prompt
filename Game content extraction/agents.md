...
- 改盲盒内容时，先看 `data/agents.md` 给出的阅读顺序，再看 `data/审查规则.md`、`data/审查文件.md`、对应类别审查文档和 `data/修改规则.md`；完成一轮替换后再读 `data/复查清单.md` 收尾复查。目标不是换词，而是解决场景错位、视觉本体重复、模板化和伪多样性；同族平移替换如 `作品记录本 -> 项目说明本` 不算修复。
