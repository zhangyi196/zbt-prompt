...
写库验收：每池 50 条且池内唯一；条目必须是真实可见物或真实物品组合，禁止靠 `收纳盒`、`整理篮`、`展示架`、`托座`、`分装盒` 等尾词批量扩写。同一基础对象不得在同一池机械重复变体；若数量不足，优先扩展不同物品族，不用同质尾词凑数。禁止依赖光影、透明感、反光、高光、发光、细线或微型痕迹作为识别点；普通杯、瓶、罐可以写，但不得强调透明或反光特征。`conditional_items`、`anchor_required_items`、`blocked_or_risky` 不再作为候选池，风险内容只进 `blocked_patterns`。
...
- `visible_small_items`：写成组可见小物，优先使用 `一组`、`一排`、`一叠`、`两枚`、`三只` 等数量前缀；禁止单个微小物、碎屑、小点、刻痕、标签、价格签和文字卡。
- `scene_expansion_items`：写 2-3 个真实物品构成的中尺度组合，格式优先为 `物品A物品B组合` 或 `物品A物品B物品C组合`；这里允许 `组合` 作为尾词，但不得围绕同一基础对象机械扩写；组合可以包含前三池物品，但条目本身必须是组合，不得直接重复前三池的单物；不得写抽象区域，不得退回大量 `组合板`、`分区垫`、`托盘`、`展示面` 变体，不写推车、抽屉柜、陈列架、货架、书桌、柜台等大型家具 / 存储主体。
