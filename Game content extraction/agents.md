...
- `data/agents.md`、`data/审查规则.md`、`data/审查文件.md`、`data/修改规则.md`：盲盒四池写库、内容审查记录和审查后替换规则；改盲盒内容前先读。
...
写库验收：每池 50 条且池内唯一；条目必须是真实可见、单点差异、边界清楚、可单独圈选的物品或成单位物品集合。修改已写库内容时，先按 `data/审查规则.md` 找问题并写入 `data/审查文件.md`，再按 `data/修改规则.md` 读取审查文件后替换，必须解决视觉本体重复、场景常识错位和同模板凑数的根因。禁止靠 `收纳盒`、`整理篮`、`展示架`、`托座`、`分装盒`、`组合` 等尾词批量扩写，也要拦住 `xxx清洁刷`、`xxx袋`、`xxx架` 等同物品族换前缀凑数。同一基础对象不得在同一类别四池机械重复；若数量不足，优先扩展不同物品族，不用同质尾词凑数。生成时先按真实物品族选物，再检查反模板规则、尾词配额和雷同密度，避免 `功能词 + 板/盘/垫/盒/包/册/卷` 成片造词。禁止依赖光影、透明感、反光、高光、发光、细线或微型痕迹作为识别点；普通杯、瓶、罐可以写，但不得强调透明或反光特征。项目禁词按测试为准，例如 `流苏` 即使在手作场景也不得进入物品池。`conditional_items`、`anchor_required_items`、`blocked_or_risky` 不再作为候选池，风险内容只进 `blocked_patterns`。
