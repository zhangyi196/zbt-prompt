每次完成会话后，同步检查本目录 `agents.md`、`README.md`，以及根目录 `agents.md`、`README.md`、`.gitignore` 是否需要随本次变更更新；这些文件必须保持精简，只记录稳定规则和必要入口。
若 `.gitignore` 命中已跟踪的缓存文件或构建缓存，必须同步取消跟踪清理。

# Game Content Extraction Guide

本目录是本地中文 `tkinter` 小工具。主入口 `内容抽取.py` 含四个工作区：`盲盒物品/动物抽取`、`人物表情抽取`、`图像抓取`、`批量重命名`。全仓高层规则见上级 `../agents.md`。

## 关键文件

- `内容抽取.py`：主 UI、抽取逻辑、历史读写、表情增强、图像抓取、批量重命名、更新检查。
- `data/blind_boxes.py`：盲盒 20 个 `场景+用途` 入口、四池内容库和兼容映射。
- `data/animals.py`、`data/item_states.py`：动物池和物品状态词。
- `draw_history.json`：物品、动物和表情抽取降权历史；表情只写 `expression_pools`。
- `expression_stats.json`：表情前置反馈统计；独立于 `draw_history.json`，只按每组输入最后一次成功抽取结果提交统计。
- `config.json`：批量重命名参数；不得接入抽取历史。
- `data/agents.md`、`data/审查规则.md`、`data/审查文件.md`、`data/审查文件/{类别} 审查文档.md`、`data/修改规则.md`、`data/复查清单.md`、`data/全局重复词定位清单.md`、`data/全局不合理命名清单.md`：盲盒写库、审查、修改、全局重复词定位、全局不合理命名定位和收尾复查规则；改盲盒内容前先读。
- `内容抽取.spec`、`installer.iss`：打包配置。

## UI 约束

- 只用原生 `tkinter/ttk`，不引入第三方 UI 依赖。
- 保持当前双栏信息结构和主要实例属性名稳定，避免打断现有事件绑定和测试脚本。
- 表情区“清空输入”只清输入，不清输出；若“自动粘贴”开启，清空后回填剪贴板文本。
- 表情区按钮顺序固定为 `表情统计`、`清空输入`、`抽取表情`、`复制结果`、`降低统计`；`降低统计` 只手动降低所选极性全部表情的已提交统计次数，不提交当前暂存结果；`复制结果` 只复制增强后文本，不包含具体表情查看框或统计摘要。

## 盲盒 / 动物抽取

- 输入语法：`盲盒编号[,盲盒编号2...][,动物类型][,子类指令1][,子类指令2...]`。
- 至少一个数字编号；动物类型最多一个：`无动物`、`地面动物`、`空中动物`、`水中动物`。
- 物品覆盖优先使用当前四池标签：`无核心物品`、`无配套物品`、`无散落小型物品`、`无场景扩展物`，以及 `配套物品+1`、`场景扩展物+1`。
- 旧兼容别名仍可识别，如 `无大型物品`、`中型物品+1`；动物覆盖仍支持 `无动物用品`、`动物本体-1`。
- 输入覆盖只影响本次抽取，不改默认值，不清空历史。
- 盲盒写库以 `data/blind_boxes.py` 为唯一事实源；抽取逻辑优先直连四池事实源，不要把第四池重新并回兼容空桶。
- 改盲盒内容时，先看 `data/agents.md` 给出的阅读顺序，再看 `data/审查规则.md`、`data/审查文件.md`、对应类别审查文档和 `data/修改规则.md`；完成一轮替换后再读 `data/复查清单.md` 收尾复查。统一目标不是换词，而是让命名先让人看到主体物、第一眼就能成像，同时解决场景错位、视觉本体重复、模板化和伪多样性；若原名过于功能化、壳体化或抽象化，可在内容清楚可见时改成“可见内容 + 主体物”，但仍要像物名，不像说明句；同族平移替换如 `作品记录本 -> 项目说明本` 不算修复；若只是给旧词前面缝场景词、口味词或材质词，如 `磅蛋糕 -> 柠檬磅蛋糕`、`茶包 -> 篝火茶包`，主词未变也视为未修复；若旧问题只被改写成另一串 `xx板 / xx架 / xx器 / xx机` 弱模板词，也视为未修复。
- 若某类问题跨类别反复出现，同步回写 `data/全局不合理命名清单.md`；它只负责全局命名问题定位，不替代单类别审查文档。

## 人物表情抽取

- 流程：读取 `极性`、`具体表情`、`单人/多人`，再从 `../组图 23 表情库.md` 查模板；只增强 `具体表情:` 字段。
- 模板兼容旧格式 `编号. 眉：...` 和新格式 `编号. 具体表情变体；字段：...`；新格式可从 `眉`、`眼`、`脸颊`、`额头`、`嘴`、`牙齿`、`舌头`、`口边`、`脸侧` 任一脸部字段开头。
- 只改脸部表情字段；不改姿态、头身朝向、四肢、衣领、头发、耳饰。
- 同栏多候选支持 `、`、`，`、`,`、`/`、`|`；校验后按历史降权随机选 1 个。
- 表情历史只写 `expression_category:*` 和 `expression_template:*`。
- 页面只显示 `具体表情查看` 框，不显示增强后文本框；`具体表情查看` 只从内部增强后文本解析 `人物定位` 和增强后的 `具体表情`，不得影响增强后文本和复制结果。
- `表情统计` 先提交当前输入最后一次成功抽取结果，再复制完整 48 类统计摘要；同一组输入多次抽取只以最后一次为准，切换输入或关闭 App 时提交未提交的最后结果。
- `降低统计` 为人工校准入口：弹窗内单选 `正向` 或 `负向` 并输入正整数次数，一次只降低所选极性的全部正式类别；只改 `expression_stats.json.committed_counts`，不得改 `draw_history.json.expression_pools`，也不得触发当前暂存结果提交。

## 图像抓取与重命名

- 图像抓取只复制，不移动、不改名、不删除源文件。
- 批量重命名参数保存到同目录 `config.json`。
- 目标文件已存在且不是当前文件时必须跳过并记录日志；不得静默覆盖、删除文件、递归处理子目录或改写不支持扩展名。
- `data/` 只放数据，不加入 UI、抽样逻辑或历史保存逻辑。

## 更新与发布

- 更新检查优先走“安装包内更新”：发现新版本且 Release 提供安装包元数据时，可下载、校验并启动安装包；若拿不到安装包元数据，再退化为提示用户打开 Releases 页面手动查看。
- 启动安装包前，必须先清理 PyInstaller 单文件进程带给子进程的 DLL 搜索路径、`_PYI_*`、`_MEIPASS2`、`TCL_LIBRARY`、`TK_LIBRARY` 等运行时环境，避免安装器回拉新版本时出现 `Failed to load Python DLL`。
- 更新流程不得直接覆盖正在运行的 exe，不得写入 `draw_history.json`，也不要长期阻塞 UI；下载和校验应放到后台线程。
- 只有发布桌面工具新版本时，才更新 `APP_VERSION`、`installer.iss` 输出名和正式 tag。

## 验证

```powershell
python -B -m py_compile '内容抽取.py'
python -B -m py_compile 'image_fetcher_ui.py' 'file_batch_renamer.py'
python -B -m unittest discover -s . -p 'test_*.py'
```

盲盒内容改动还需额外运行：

```powershell
python -B 'test_blind_box_content_model.py'
```

并按 `data/复查清单.md` 补做人工复查；`data/修改规则.md` 负责修改边界，`data/复查清单.md` 负责收尾确认，至少确认四池数量、池内唯一、第四池跨池重叠、禁词、尾词配额和模板密度没有回退。

## 打包

```powershell
pyinstaller --clean --noconfirm '内容抽取.spec'
& "$env:LOCALAPPDATA\Programs\Inno Setup 6\ISCC.exe" 'installer.iss'
pyi-archive_viewer 'dist\内容抽取.exe' --list | Select-String -Pattern 'tkinter|_tkinter|tcl86t|tk86t'
```
