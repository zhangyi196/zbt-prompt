...
- `主图 第一步.md`：主图候选生成前置规则；先判断画面是否真的需要新增内容，若不需要则固定输出 `不需要新增物品`，若需要再输出 2 个盲盒类别编号、动物字段、禁用字段和补偿字段。
...
`主图 第一步.md` 的盲盒类别库也已同步为这 20 个新入口，并补充每类物品速查和新的场景硬约束；该提示词只输出编号和控制字段，不输出完整物品清单。
