...
- `美式卡通具体表情写法.md`：人物表情库趣味化重构草案，说明“简洁情绪类别 + 8 个具体卡通脸部变体”的写法。
- `表情库 2.0.0版本.md`：人物表情库 2.0.0 可审目录草案，只列类别与变体，尚未替代当前事实源。
...
- `.vscode/settings.json`：工作区 Python 解释器固定到 `Game content extraction/.venv/Scripts/python.exe`。
...
桌面工具开发、验证和打包默认走项目内 `Game content extraction/.venv/`。本地构建缓存 `Game content extraction/build/`、`Game content extraction/dist/` 和安装包产物 `Game content extraction/release/*.exe` 默认保持忽略。安装包固定输出到 `Game content extraction/release/`；检查更新默认走“安装包内更新”。若为验证升级链路临时发布更高版本 Release，验证完成后应删除该临时 Release 与本地同名安装包。桌面工具版本、安装包和更新检查细节以 `Game content extraction/` 下文档为准。
